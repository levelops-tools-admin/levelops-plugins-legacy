...
def collect_subs(title, line, sub_contents, fields):
  for field in fields:
    if field == title:
      continue
    if line.startswith(field):
      return False, ' '.join(sub_contents).replace(title + ' ', '')
...
def finding_parser(last_line: str, contents: list):
  ...
  for line in contents:
    ...
    if (collecting and collecting_description) or (not collecting and (line.startswith('Description') or previous_line.startswith('Description')) and (not description or len(description) < 1)):
      collecting_description,  description = collect_subs('Description', line, sub_contents, fields)
    ...
    if (collecting and collecting_location) or (not collecting and (line.startswith('Location') or previous_line.startswith('Location')) and (not location or len(location) < 1)):
      collecting_location,  location = collect_subs('Location', line, sub_contents, fields)
    ...
    if (collecting and collecting_impact_description) or (not collecting and (line.startswith('Impact') or previous_line.startswith('Impact')) and (not impact_description or len(impact_description) < 1)):
      collecting_impact_description,  impact_description = collect_subs('Impact', line, sub_contents, fields)
    ...
    if (collecting and collecting_recommendation) or (not collecting and (line.startswith('Recommendation') or previous_line.startswith('Recommendation')) and (not recommendation or len(recommendation) < 1)):
      collecting_recommendation,  recommendation = collect_subs('Recommendation', line, sub_contents, fields)
    ...
    if (collecting and collecting_steps) or (not collecting and (line.startswith('Reproduction Steps') or previous_line.startswith('Reproduction Steps')) and (not steps or len(steps) < 1)):
      collecting_steps, steps = collect_subs('Reproduction Steps', line, sub_contents, fields)
      if collecting_steps:
        collecting = True
        previous_line = line
        continue
    ...
    # elif line.startswith('Risk') or previous_line.startswith('Risk') and (not risk or len(risk) < 1):
    #   risk = line.replace('Risk ', '').replace('Risk', '').strip()
    ...
    # elif line.startswith('Location') or previous_line.startswith('Location') and (not location or len(location) < 1):
    ...
    # elif line.startswith('Impact') or previous_line.startswith('Impact') and (not status or len(impact_description) < 1):
    #   impact_description = line.replace('Impact ', '').replace('Impact', '').strip()
    # elif line.startswith('Recommendation') or previous_line.startswith('Recommendation') and (not status or len(recommendation) < 1):
    #   recommendation = line.replace('Recommendation ', '').replace('Recommendation', '').strip()
    # elif line.startswith('Reproduction Steps') or previous_line.startswith('Reproduction Steps') and (not status or len(recommendation) < 1):
    #   steps = line.replace('Reproduction Steps ', '').replace('Reproduction Steps', '').strip()
    # if line.startswith('Description') or previous_line.startswith('Description') and (not status or len(description) < 1):
...
def ignore_line_evaluator(line):
  return page_footer_matcher.match(line) is not None
